...
api = Api(app, authorizations=authorizations, version='1.0', title='IAM Service API docs',
    description='IAM Service - CLO5 API.',
    doc='/docs',
        ...
        if entity == 'users':
            ...
            if db.mongo.db[entity].count_documents({}) != 0: #and db.find(entity, {'email': user['email']}):
                ...
                for data in dbstuff[entity]:
                    if data['ownerUID']:
                        app.logger.info(f'Finding {entity} {data["ownerUID"]} UID')
                        ouid = db.find('users', {'email': data['ownerUID']}) or None
                        if not ouid:
                            continue
                        data['ownerUID'] = ouid[0]['_id'] or None
...
if __name__ == '__main__':
    app.run(use_reloader=True, host='0.0.0.0', port=os.getenv('APP_PORT',5051))
